...
def test_tracker_locks_after_eight_measure_spaced_taps() -> None:
    tracker = TapTempoTracker(required_taps=8, min_tap_interval=0.2)
    ...
    assert result.snapshot.tap_count == 8
...
def test_tracker_ignores_accidental_double_tap() -> None:
    tracker = TapTempoTracker(required_taps=8, min_tap_interval=0.3)
